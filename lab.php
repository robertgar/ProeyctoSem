...
<div class="content">
 <br><br>
  ...
  <div class="panel panel-success">
    ...
    <tbody>
      <?php
      $sql = "SELECT t.id AS id,p.dpi_pa AS dpi_pa, p.nom_pa AS nom_pa, en.nom_en AS nom_en, ex.nombre AS nombre_ex, de.resultado_ex AS resultado_ex
FROM tbl_serv as t 
  ...
  while($row =  mysqli_fetch_array($query_so)) {
    ...
    ?>
           ...
           <td><?php echo $row["nombre_ex"];?></td>
            ...
            if ($row["resultado_ex"]=="") {
              ?>
              <a class="btn btn-warning">Pendiente</a></td>
              <?php
            } else {
              ?>
              <a href="verlab.php?id=<?php echo $row["id"];?>" class="btn btn-info">
